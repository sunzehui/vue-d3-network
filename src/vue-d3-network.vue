...
const d3 = Object.assign({}, forceSimulation, drag, { select }, zoom);
  ...
  methods: {
    ...
    drag() {
      ...
      function dragged(event) {
        console.log(event);
        d3.select(this)
          .attr("cx", (this.x = event.x))
          .attr("cy", (this.y = event.y));
      }
    ...
    animate() {
      ...
      // d3.select(".ground").attr("cursor", "grab").call(this.drag());
      const vm = this;
      function zoomed({ transform }) {
        // this.mouseOfst.x = transform.x;
        // this.mouseOfst.y = transform.y;
